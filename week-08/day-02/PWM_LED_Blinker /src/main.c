...
/* pin config: we cannot use the green user LED because it's not connected to an
 * output compare channel so we use PA15 */
...
void init_LED() {
  ...
  PA15_LED_config.Mode =
      GPIO_MODE_AF_PP; /* configure as output, in PUSH-PULL mode */
  PA15_LED_config.Pull = GPIO_NOPULL;
  PA15_LED_config.Speed = GPIO_SPEED_HIGH;
  PA15_LED_config.Alternate = GPIO_AF1_TIM2; /* we need this alternate function
                                                to use TIM2 in OC mode */
...
int main(void) {
  ...
  while (1) {
    ...
    __HAL_TIM_SET_COMPARE(&timer_handle, TIM_CHANNEL_1, dim);
